pipeline {
    ...
    environment {
        PROD_ENV     = "/opt/env/.env.ustatop_backend"
        IMAGE_NAME   = "ustatop_backend"
        TEST_TAG     = "test"
        PROD_TAG     = "latest"
        CONTAINER_DB = "ustatop_backend_db_test"
        CONTAINER_WEB = "ustatop_backend_web_test"
        CONTAINER_REDIS = "ustatop_backend_redis_test"
        STACK_NAME = "ustatop_backend"
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git branch: 'main', credentialsId: 'ssh', url: 'git@github.com:JscorpTech/ustatop_backend.git'
            }
        }
        stage('Build Image') {
            steps {
